...
public class Myz {
    ...
    public static void main(String[] argv) {
         ...
         */
        /*
         * double[][] A = {{ 0.0, 1.0, 4.0, 9.0} ,{1.0, 0.0, 7.0, 5.0} ,{1.0/4.0,
         * 1.0/7.0, 0.0, 1.0/40.0} ,{1.0/9.0, 1.0/5.0, 40.0, 0.0}};
         */
        double[][] A = { { 0.0, 5.0, 5.0, 9.0 }, { 1.0 / 5.0, 0.0, 1.0 / 7.0, 5.0 }, { 1.0 / 5.0, 7.0, 0.0, 1.0 / 5.0 },
                { 1.0 / 9.0, 1.0 / 5.0, 5.0, 0.0 } };
        ...
        while (true) {
            for (int i = 0; i < 4; i++) {
                ...
                for (int j = 0; j < 4; j++)
                    if (j != i)
                        c[i][j] = (tmpC - Math.abs(eigen[i] - A[i][j] * eigen[j])) / tmpC / (4.0 - 2.0);
            }
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++) {
                    cA[i][j] = c[i][j] * A[i][j];
            ...
            System.out.println(";;; " + CIs[0] + " ;;; " + CIs[1]);
            normalize(eigen);
            stdout(eigen);
